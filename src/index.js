...
import { saveProjects, loadProjects, saveNotes, loadNotes } from "./storage.js";
...
const INFO_ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free v6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM216 336l24 0 0-64-24 0c-13.3 0-24-10.7-24-24s10.7-24 24-24l48 0c13.3 0 24 10.7 24 24l0 88 8 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-80 0c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-208a32 32 0 1 1 0 64 32 32 0 1 1 0-64z"/></svg>`;
...
content.addEventListener("input", (event) => {
    ...
    const note = findNote(event.target.closest(".note").dataset.id);
    if (!note) {
        return;
    }

    note.updateText(event.target.textContent);
    saveNotesSoon();
});
...
function createProjectTab(project) {
    ...
    tab.append(initial, name, count, remove);
    tab.classList.toggle("is-active", activeView === "project" && project.id === activeProjectId);
    return tab;
}
...
function createItemElement(item) {
    ...
    priority.style.backgroundColor = (PRIORITIES[item.priority] ?? PRIORITIES.medium).color;
    ...
    element.append(priority, checkbox, title, info, dueDate, edit, remove);
...
function refreshItemElement(item) {
    ...
    if (existing) {
        existing.replaceWith(createItemElement(item));
    }
}
